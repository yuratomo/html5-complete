...
let g:css_complete_item_len = 30
...
function! csscore#complete(findstart, base)
  if a:findstart
    let line = line('.')
    let cur = col('.') - 1
    ...
    if s:complete_mode == s:MODE_NAMESPACE
      call csscore#ns_completion(a:base, res)
    ...
    elseif s:complete_mode == s:MODE_STATIC
      ...
      if len(s:parts) >= 1 
        ...
        for ns in csscore#getNamespaces()
          if ns =~ '^' . variable
            let compitem = ns[start : ]
            call add(res, csscore#ns_to_compitem(compitem))
          endif
          if ns == type
            let match_ns = 1
          endif
        ...
        elseif csscore#isEnumExist(s:parts[0])
...
function! csscore#ns_completion(base, res)
  for ns in s:namespace
    if ns =~ '^' . a:base
      call add(a:res, csscore#ns_to_compitem(ns))
    endif
  endfor
endfunction
...
function! csscore#class_completion(base, res)
...
function! csscore#enum_completion(base, res)
...
function! s:class_member_completion(base, res, type)
  ...
  for part in parts
    ...
    if !csscore#isClassExist(class)
      if !csscore#isEnumExist(class)
"       let item = csscore#getTag(class)
"       if empty(item)
"         if exists('item')
"           unlet item
"         endif
...
"       endif
        ...
        let item = csscore#getEnum(class)
      endif
    else
      let item = csscore#getClass(class)
    endif
    ...
    while 1
      ...
      if has_key(item, 'extend')
        if csscore#isClassExist(item.extend)
          let item = csscore#getClass(item.extend)
        else
"         let item = csscore#getTag(item.extend)
"         if empty(item)
"           unlet item
            break
"         endif
  ...
  if exists('item')
    ...
    elseif a:type == 1
      ...
      if !has_key(s:primitive_dict, item.name)
        let newitem = csscore#member_to_compitem('new ' . item.name . '(', {})
        let newitem.menu = 'create new instance'
        call add(a:res, newitem)
        let newitem = csscore#member_to_compitem('(' . item.name . ')', {})
        let newitem.menu = 'for cast'
        call add(a:res, newitem)
      endif
      if empty(a:res)
        call csscore#class_completion(a:base, a:res)
      endif
    else
...
function! s:find_type(start_line, var)
  ...
  if csscore#isClassExist(result.class)
...
"   endif
  ...
  for member in item.members
   ...
   if member.name =~ '^' . a:base
      call add(a:res, csscore#member_to_compitem(item.name, member))
    endif
  endfor
...
function! s:abbr(str)
  if len(a:str) > g:css_complete_item_len
    return a:str[0 : g:css_complete_item_len] . '...'
  endif
  return a:str
endfunction

function! csscore#member_to_compitem(class, member)
  if empty(a:member)
    return {
      \ 'word' : a:class,
      \ 'abbr' : s:abbr(a:class),
...
function! csscore#ns_to_compitem(ns)
  return {
    \ 'word' : a:ns,
    \ 'menu' : 'namespace',
    \ 'kind' : 't',
...
let s:class = {}
function! csscore#class(name, extend, members)
  let s:class[ a:name ] = s:def_class(a:name, a:extend, a:members)
  if exists('s:parent') && index(s:parent.members, a:name) == -1
    call add(s:parent.members, csscore#prop(a:name, a:name))
  endif
endfunction

let s:namespace = []
function! csscore#namespace(ns)
  try
    unret s:parent
  catch /.*/
  endtry
  ...
  for part in parts
    ...
    "  call add(s:parent.members, csscore#prop(part, part))
    "endif

    if !csscore#isClassExist(part)
      call s:namespace_item(part, '', [])
    endif

    " last namespace (for csscore#class)
    let s:parent = csscore#getClass(part)
  endfor
endfunction

let s:keyword = []
function! csscore#keyword(name, detail)
  call add(s:keyword, 
    \ {
    \ 'type'      : s:TYPE_KEYWORD,
    \ 'name'      : a:name,
...
function! csscore#method(name, detail, class)
  return {
    \ 'type'   : s:TYPE_METHOD,
    \ 'kind'   : 'f', 
    \ 'name'   : a:name,
...
function! csscore#prop(name, class)
  return {
    \ 'type'   : s:TYPE_PROP,
    \ 'kind'   : 'm', 
    \ 'name'   : a:name,
...
function! csscore#attr(name, class)
  return {
    \ 'type'   : s:TYPE_ATTR,
    \ 'kind'   : 'm', 
    \ 'name'   : a:name,
...
function! csscore#const(name, value)
  return {
    \ 'type'   : s:TYPE_CONST,
    \ 'kind'   : 'm', 
    \ 'name'   : a:name,
...
function! csscore#field(name, class)
  return {
    \ 'type'   : s:TYPE_FIELD,
    \ 'kind'   : 'v', 
    \ 'name'   : a:name,
...
function! csscore#event(name, class)
  return {
    \ 'type'   : s:TYPE_EVENT,
    \ 'kind'   : 'f',
    \ 'name'   : a:name,
...
let s:enum = {}
function! csscore#enum(name, members)
  let s:enum[ a:name ] = {
    \ 'type'   : s:TYPE_ENUM,
    \ 'name'   : a:name,
    \ 'kind'   : 't',
    ...
    \ 'detail' : '',
    \ }
  return s:enum[ a:name ]
endfunction

let s:binding = {}
function! csscore#bind(name, members)
...
function! csscore#isMethod(member)
...
function! csscore#isConst(member)
...
function! csscore#isAttr(member)
...
function! csscore#isEvent(member)
...
function! csscore#isClassExist(name)
  return has_key(s:class, a:name)
endfunction

function! csscore#getClass(name)
  return get(s:class, a:name)
endfunction

function! csscore#getSuperClassList(name, list)
  if !csscore#isClassExist(a:name)
    return
  endif

  let item = csscore#getClass(a:name)
  if item.extend != '' && item.extend != '-'
    call add(a:list, item.extend)
    call csscore#getSuperClassList(item.extend, a:list)
  endif
endfunction

" その都度tagsからロードしていると遅いので、
" タグ → s:class にロードして使うようにする
" function! csscore#getTag(name)
...
"       let ttype = name
"     endif
"     call add(class.members, csscore#prop(name, ttype))
"   endfor
"   return class
" endfunction

function! csscore#isBindingExist(name)
  return has_key(s:binding, a:name)
endfunction

function! csscore#getBinding(name)
  return get(s:binding, a:name)
endfunction

function! csscore#isEnumExist(name)
  return has_key(s:enum, a:name)
endfunction

function! csscore#getEnum(name)
  return get(s:enum, a:name)
endfunction

function! csscore#getNamespaces()
  return s:namespace
endfunction
...
function! csscore#showRef()
  if !exists('g:csscore#statusline')
    let g:csscore#statusline = &statusline
  endif
  ...
  \ 'line'  : line('.'),
  \ }
  call csscore#nextRef()

  augroup csscore
    au!
    au InsertLeave  <buffer> call csscore#clearRef()
    au CursorMovedI <buffer> call csscore#checkLineForRef()
  augroup END
  return ""
endfunction
...
function! csscore#nextRef()
  return s:prevNextRef(1)
endfunction
function! csscore#prevRef()
  return s:prevNextRef(-1)
endfunction
...
function! csscore#checkLineForRef()
  if exists("b:ref")
    if b:ref.line != line('.')
      call csscore#clearRef()
    endif
  endif
endfunction

function! csscore#reshowRef()
  if exists("b:ref")
    if b:ref.line == line('.')
...
function! csscore#clearRef()
  let &l:statusline = g:csscore#statusline
  augroup csscore
    au!
    au InsertEnter  <buffer> call csscore#reshowRef()
  augroup END
endfunction

function! csscore#balloon()
      ...
      elseif has_key(titem, "class") && ((titem.kind == "f" || titem.kind == "m" || titem.kind == "p"))
        ...
        if index(defs[cname].member_names, mname) == -1
          ...
          if has_key(member, 'signature')
            ...
            let item = csscore#method(mname . '(', signature[1:], ttype)
            let mname = mname . signature
          else
            let item = csscore#prop(mname, ttype)
          endif
...
" load autoload/css3/.vim
function! csscore#load(sub)
  let rtp = filter(split(&runtimepath, ','), 'v:val =~ a:sub')
  let files = split(globpath(join(rtp, ','), 'autoload/css3/*.vim'), '\n')
  for file in files
    if file
      continue
    endif
  ...
  call s:msg('loadded')
  call remove(g:csscore#delay_dirs, a:sub)
endfunction
...
if !exists('s:dictionary_loaded')
  ...
  " delay directories
  if !exists('g:csscore#delay_dirs')
    let g:csscore#delay_dirs = []
  end
  let rtp = split(&runtimepath, ',')
  for dir in g:csscore#delay_dirs
